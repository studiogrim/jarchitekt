...
console.log("fghgyjhg");
            ...
            var fileFormat = elem.attr(options.sourceAttr).split('.');
            fileFormat = fileFormat[fileFormat.length - 1];

            if(fileFormat=="mp4"){
                curImg = $( '<video controls />' )
                    .hide()
                    .attr('data-scale', 1)
                    .attr('data-translate-x', 0)
                    .attr('data-translate-y', 0);
                curImg.append('<source src="'+elem.attr(options.sourceAttr)+'" type="video/mp4">');
			}else{
                curImg = $( '<img/>' )
                    .hide()
		...
		adjustImage = function(dir){
            ...
            if(curImg.is("video")){
            	...
            	tmpImage.append('<source src="'+curImg.find("source:first").attr("src")+'" type="video/mp4">');
            }else{
			...
			if(curImg.is("img")){
                ...
                if(loaded.indexOf(curImg.find('source').attr( 'src' )) == -1){
                    loaded.push(curImg.find('source').attr( 'src' ));
                }
               /* var imageWidth	 = tmpImage.width,
                    imageHeight	 = tmpImage.height;*/
                var imageWidth	 = 1000,
